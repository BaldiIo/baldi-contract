...
contract Liquidations is Owned, MixinResolver, MixinSystemSettings, ILiquidations {
    ...
    function flagAccountForLiquidation(address account) external rateNotInvalid("BADI") {
    ...
    // Check SNX rate is not stale
    function checkAndRemoveAccountInLiquidation(address account) external rateNotInvalid("BADI") {
        systemStatus().requireSystemActive();
